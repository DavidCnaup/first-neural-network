...
# <span id="intro">
# 下图展示的是数据集中前 10 天左右的骑车人数（某些天不一定是 24 个条目，所以不是精确的 10 天）。你可以在这里看到每小时租金。这些数据很复杂！周末的骑行人数少些，工作日上下班期间是骑行高峰期。我们还可以从上方的数据中看到温度、湿度和风速信息，所有这些信息都会影响骑行人数。你需要用你的模型展示所有这些数据。</span>
...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.derivation_function = lambda x : x * (1 - x)
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #output_error_term = error * deri_func(final_outputs)
            #because f(x) = x
            #output_error_term = error
            #hidden_error = output_error.dot(weight_h_o) = error * weight_h_o
            ...
            hidden_error_term = hidden_error * self.derivation_function(hidden_outputs)
...
cost_time = time.time()-last
print('\nCost time {:.2f}: '.format(cost_time))
...
plt.legend()
plt.title('Cost time {:.2f}s: '.format(cost_time))
_ = plt.ylim()
...
# 
# #### 请将你的答案填写在下方
# 模型对预测数据效果比较满意，从图中看到Dec-21之前的数据预测值和实际都很吻合。但是Dec-21之后的数据差异性很大，因为这部分数据正好处在假期，查看美国的寒假时间：
# >如果是Quarter制度：从12月10日左右到1月 5日左右。
# >如果是一年两学期的：从12月12日左右到1月17日左右
# 
# [数据简介里有说明](#intro)，周末的骑行人数少些，工作日上下班期间是骑行高峰期，所以同样在假期里，这些数据实际都低于我们模型的预测值。
# 
